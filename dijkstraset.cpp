...
void dijkstra( ll s )
{
    ...
    pq.insert({0,s});
    dist[s] = 0;
    ll ansmax = INT_MIN, cnt = 0;
    while ( !pq.empty() )
    {
        ll node = (*pq.begin()).sec;
        ll d = (*pq.begin()).fi;
        visited[node] = 1;
        if ( d > ansmax )
            ansmax = d, cnt = 1;
        ...
        {
            if ( !visited[it.fi] and ((d+it.sec)<dist[it.fi]) )
            {
                if ( dist[it.fi] != INT_MAX )
                    pq.erase({dist[it.fi],it.fi}); 
                dist[it.fi] = d + it.sec;
                pq.insert({d+it.sec,it.fi});
            }
        }
    }
    cout << ansmax <<" " << cnt <<"\n";
